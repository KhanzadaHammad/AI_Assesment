...
A directed acyclic graph (DAG) built with LangGraph.
...
Tools DoSchema Validation: Use Pydantic (SerpQueryArgs) to validate incoming arguments (keywords, location code, language) before making requests.
API Wrapper: Interface directly with DataForSEO (or its mock stub) per individual search call.
Resilience & Retry: Handle transient failures with exponential backoff and randomized jitter.
Error Classification: Distinguish retryable network/rate-limit errors from non-retryable errors, routing persistent failures to a fallback recovery node.
Call Counts: LLM & EmbedderFor a single pipeline run with 3 planned queries:
      LLM Calls (3 total): Query Planner Node (1 call)  Analysis / Synthesis Node (1 call)  Report Node (1 call)
      Embedder Calls (2 total):
            Document Indexing: Embeds the extracted SERP snippets into the vector store.Query Search: Embeds the target domain query to retrieve top-k similar ranking snippets.